...
static void dleq_nonce_bitflip(unsigned char **args, size_t n_flip, size_t n_bytes) {
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_dleq_nonce(&k1, args[0], args[1], args[2], args[3], NULL, args[4]) == 1);
    testrand_flip(args[n_flip], n_bytes);
    CHECK(rustsecp256k1zkp_v0_11_0_dleq_nonce(&k2, args[0], args[1], args[2], args[3], NULL, args[4]) == 1);
...
static void dleq_tests(void) {
    ...
    rand_point(&gen2);
    rand_scalar(&sk);
    rustsecp256k1zkp_v0_11_0_dleq_pair(&CTX->ecmult_gen_ctx, &p1, &p2, &sk, &gen2);
    CHECK(rustsecp256k1zkp_v0_11_0_dleq_prove(CTX, &s, &e, &sk, &gen2, &p1, &p2, NULL, NULL) == 1);
    CHECK(rustsecp256k1zkp_v0_11_0_dleq_verify(&s, &e, &p1, &gen2, &p2) == 1);

    {
        rustsecp256k1zkp_v0_11_0_scalar tmp;
        rustsecp256k1zkp_v0_11_0_scalar_set_int(&tmp, 1);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_verify(&tmp, &e, &p1, &gen2, &p2) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_verify(&s, &tmp, &p1, &gen2, &p2) == 0);
    }
    {
        rustsecp256k1zkp_v0_11_0_ge p_tmp;
        rand_point(&p_tmp);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_verify(&s, &e, &p_tmp, &gen2, &p2) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_verify(&s, &e, &p1, &p_tmp, &p2) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_verify(&s, &e, &p1, &gen2, &p_tmp) == 0);
    }
    {
        rustsecp256k1zkp_v0_11_0_ge p_inf;
        rustsecp256k1zkp_v0_11_0_ge_set_infinity(&p_inf);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_prove(CTX, &s, &e, &sk, &p_inf, &p1, &p2, NULL, NULL) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_prove(CTX, &s, &e, &sk, &gen2, &p_inf, &p2, NULL, NULL) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_dleq_prove(CTX, &s, &e, &sk, &gen2, &p1, &p_inf, NULL, NULL) == 0);
    }

    /* Nonce tests */
    rustsecp256k1zkp_v0_11_0_scalar_get_b32(sk32, &sk);
    CHECK(rustsecp256k1zkp_v0_11_0_eckey_pubkey_serialize(&gen2, gen2_33, &pubkey_size, 1));
    CHECK(rustsecp256k1zkp_v0_11_0_eckey_pubkey_serialize(&p1, p1_33, &pubkey_size, 1));
    CHECK(rustsecp256k1zkp_v0_11_0_eckey_pubkey_serialize(&p2, p2_33, &pubkey_size, 1));
    CHECK(rustsecp256k1zkp_v0_11_0_dleq_nonce(&k, sk32, gen2_33, p1_33, p2_33, NULL, NULL) == 1);
...
static void test_ecdsa_adaptor_spec_vectors_check_serialization(const unsigned char *adaptor_sig162, int expected) {
    ...
    if (expected == 1) {
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_sig_serialize(buf, &r, &rp, &sp, &dleq_proof_e, &dleq_proof_s) == 1);
...
static void ecdsa_adaptor_test_sha256_eq(const rustsecp256k1zkp_v0_11_0_sha256 *sha1, const rustsecp256k1zkp_v0_11_0_sha256 *sha2) {
...
static void run_nonce_function_ecdsa_adaptor_tests(void) {
     ...
     * state. */
    rustsecp256k1zkp_v0_11_0_sha256_initialize_tagged(&sha, tag, sizeof(tag));
    ...
    * state. */
    rustsecp256k1zkp_v0_11_0_sha256_initialize_tagged(&sha, aux_tag, sizeof(aux_tag));
    ...
    * state. */
    rustsecp256k1zkp_v0_11_0_sha256_initialize_tagged(&sha, dleq_tag, sizeof(dleq_tag));
...
static void adaptor_tests(void) {
    ...
    CHECK(rustsecp256k1zkp_v0_11_0_ec_pubkey_create(CTX, &enckey, deckey) == 1);
    CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_encrypt(CTX, adaptor_sig, seckey, &enckey, msg, NULL, NULL) == 1);

    {
        /* Test overflowing seckey */
        memset(big, 0xFF, 32);
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_encrypt(CTX, adaptor_sig, big, &enckey, msg, NULL, NULL) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(adaptor_sig, zeros162, sizeof(adaptor_sig)) == 0);

        /* Test different nonce functions */
        memset(adaptor_sig, 1, sizeof(adaptor_sig));
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_encrypt(CTX, adaptor_sig, seckey, &enckey, msg, ecdsa_adaptor_nonce_function_failing, NULL) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(adaptor_sig, zeros162, sizeof(adaptor_sig)) == 0);
        memset(&adaptor_sig, 1, sizeof(adaptor_sig));
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_encrypt(CTX, adaptor_sig, seckey, &enckey, msg, ecdsa_adaptor_nonce_function_0, NULL) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(adaptor_sig, zeros162, sizeof(adaptor_sig)) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_encrypt(CTX, adaptor_sig, seckey, &enckey, msg, ecdsa_adaptor_nonce_function_overflowing, NULL) == 1);
        CHECK(rustsecp256k1zkp_v0_11_0_memcmp_var(adaptor_sig, zeros162, sizeof(adaptor_sig)) != 0);
    }
    {
        ...
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_sig_serialize(adaptor_sig_tmp, &r, &rp, &sp, &dleq_proof_e, &dleq_proof_s) == 1);
        ...
        rustsecp256k1zkp_v0_11_0_ge_set_infinity(&p_inf);
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_sig_serialize(adaptor_sig_tmp, &p_inf, &rp, &sp, &dleq_proof_e, &dleq_proof_s) == 0);
        CHECK(rustsecp256k1zkp_v0_11_0_ecdsa_adaptor_sig_serialize(adaptor_sig_tmp, &r, &p_inf, &sp, &dleq_proof_e, &dleq_proof_s) == 0);
    }
    {
