...
SECP256K1_INLINE static void rustsecp256k1zkp_v0_11_0_borromean_hash(unsigned char *hash, const unsigned char *m, size_t mlen, const unsigned char *e, size_t elen,
 size_t ridx, size_t eidx) {
    ...
    rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_en, e, elen);
    rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_en, m, mlen);
    rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_en, ring, 4);
    rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_en, epos, 4);
    rustsecp256k1zkp_v0_11_0_sha256_finalize(&sha256_en, hash);
 ...
 */
int rustsecp256k1zkp_v0_11_0_borromean_verify(rustsecp256k1zkp_v0_11_0_scalar *evalues, const unsigned char *e0,
 const rustsecp256k1zkp_v0_11_0_scalar *s, const rustsecp256k1zkp_v0_11_0_gej *pubs, const size_t *rsizes, size_t nrings, const unsigned char *m, size_t mlen) {
    ...
    for (i = 0; i < nrings; i++) {
        ...
        rustsecp256k1zkp_v0_11_0_borromean_hash(tmp, m, mlen, e0, 32, i, 0);
        ...
        for (j = 0; j < rsizes[i]; j++) {
            ...
            rustsecp256k1zkp_v0_11_0_eckey_pubkey_serialize(&rge, tmp, &size, 1);
            if (j != rsizes[i] - 1) {
                rustsecp256k1zkp_v0_11_0_borromean_hash(tmp, m, mlen, tmp, 33, i, j + 1);
                rustsecp256k1zkp_v0_11_0_scalar_set_b32(&ens, tmp, &overflow);
            } else {
                rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_e0, tmp, size);
            }
            count++;
        }
    }
    rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_e0, m, mlen);
    rustsecp256k1zkp_v0_11_0_sha256_finalize(&sha256_e0, tmp);
    rustsecp256k1zkp_v0_11_0_sha256_clear(&sha256_e0);
    return rustsecp256k1zkp_v0_11_0_memcmp_var(e0, tmp, 32) == 0;
}

int rustsecp256k1zkp_v0_11_0_borromean_sign(const rustsecp256k1zkp_v0_11_0_ecmult_gen_context *ecmult_gen_ctx,
 ...
 const size_t *rsizes, const size_t *secidx, size_t nrings, const unsigned char *m, size_t mlen) {
    ...
    for (i = 0; i < nrings; i++) {
        ...
        for (j = secidx[i] + 1; j < rsizes[i]; j++) {
            rustsecp256k1zkp_v0_11_0_borromean_hash(tmp, m, mlen, tmp, 33, i, j);
            rustsecp256k1zkp_v0_11_0_scalar_set_b32(&ens, tmp, &overflow);
    ...
    rustsecp256k1zkp_v0_11_0_sha256_write(&sha256_e0, m, mlen);
    rustsecp256k1zkp_v0_11_0_sha256_finalize(&sha256_e0, e0);
    rustsecp256k1zkp_v0_11_0_sha256_clear(&sha256_e0);
    count = 0;
    for (i = 0; i < nrings; i++) {
        VERIFY_CHECK(INT_MAX - count > rsizes[i]);
    ...
    rustsecp256k1zkp_v0_11_0_memclear(tmp, 33);
    return 1;
}
